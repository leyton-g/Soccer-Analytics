...
# ---------- Data source toggle ----------
import os
REAL_DATA_AVAILABLE = os.path.exists("real_player_stats.csv")

st.sidebar.header("Data Source")
if REAL_DATA_AVAILABLE:
    use_real = st.sidebar.toggle("Use real FBref data", value=True)
else:
    use_real = False
    st.sidebar.caption("real_player_stats.csv not found — using sample data. Run fetch_real_data.py and place the CSV next to app.py to enable real data.")

# ---------- Load data ----------
df = get_processed_data(use_real=use_real)

if use_real:
    # Real-data schema: no xG/xA/key-passes/dribbles available at this level,
    # so the radar/comparison metrics are built from what's actually real.
    RADAR_METRICS = {
        "Goals p90": "goals_p90",
        "Assists p90": "assists_p90",
        "Shots p90": "shots_p90",
        "Shots on Target p90": "shots_on_target_p90",
        "Crosses p90": "crosses_p90",
    }
else:
    RADAR_METRICS = {
        "Goals p90": "goals_p90",
...
if use_real:
    st.caption("✅ Real season data pulled from FBref via `soccerdata` (goals, assists, shots, crosses, etc.). Expected-goals and passing/dribbling data aren't available at this data tier.")
else:
    st.caption("⚠️ Sample/illustrative dataset — not real stats. Toggle 'Use real FBref data' in the sidebar once you've run fetch_real_data.py.")
...
# ================= VIEW 4: SHOOTING EFFICIENCY =================
elif view == "Shooting Efficiency":
    if use_real:
        st.subheader("Shooting Efficiency: Goals vs. Shots")
        st.caption("Points above the dashed line convert shots into goals at a higher rate than the group average. This uses real shots/goals — expected goals (xG) data isn't available at this data tier, so this is actual finishing rate rather than xG overperformance.")

        avg_conversion = df["goals"].sum() / df["shots"].sum() if df["shots"].sum() > 0 else 0
        max_val = max(df["shots"].max(), 1)

        fig = px.scatter(
            df, x="shots", y="goals", color="league", size="minutes",
            hover_name="player", hover_data={"team": True, "shots": True, "goals": True},
            labels={"shots": "Total Shots", "goals": "Actual Goals"},
        )
        fig.add_shape(
            type="line", x0=0, y0=0, x1=max_val, y1=max_val * avg_conversion,
        ...
        for p, color in [(player_1, "black"), (player_2, "red")]:
            ...
            fig.add_annotation(
                x=prow["shots"], y=prow["goals"], text=p, showarrow=True,
                arrowhead=2, ax=20, ay=-30, font=dict(size=11, color=color),
            )
        st.plotly_chart(fig, use_container_width=True)

        df_eff = df.copy()
        df_eff["conversion_rate"] = (df_eff["goals"] / df_eff["shots"].replace(0, pd.NA)).round(3)
        top_eff = df_eff.sort_values("conversion_rate", ascending=False).head(5)[["player", "team", "goals", "shots", "conversion_rate"]]
        st.markdown("**Best conversion rate (goals per shot)**")
