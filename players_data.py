...
RAW_DATA = [
    ...
    {"player": "Lamine Yamal", "team": "Barcelona", "league": "La Liga", "position": "FW",
     "minutes": 2800, "goals": 9, "assists": 13, "xg": 8.5, "xa": 10.2,
     "shots": 55, "key_passes": 75, "dribbles_completed": 110,
     "progressive_carries": 160, "touches_in_box": 120},

    {"player": "Kylian Mbappe", "team": "Real Madrid", "league": "La Liga", "position": "FW",
     "minutes": 2700, "goals": 26, "assists": 7, "xg": 23.0, "xa": 6.0,
     "shots": 100, "key_passes": 40, "dribbles_completed": 70,
     "progressive_carries": 140, "touches_in_box": 200},

    {"player": "Nico Williams", "team": "Athletic Bilbao", "league": "La Liga", "position": "FW",
     "minutes": 2600, "goals": 8, "assists": 10, "xg": 7.5, "xa": 8.8,
     "shots": 48, "key_passes": 55, "dribbles_completed": 100,
     "progressive_carries": 145, "touches_in_box": 95},
    ...
    {"player": "Alexander Isak", "team": "Newcastle United", "league": "Premier League", "position": "FW",
     "minutes": 2500, "goals": 24, "assists": 5, "xg": 21.0, "xa": 4.5,
     "shots": 95, "key_passes": 25, "dribbles_completed": 20,
     "progressive_carries": 40, "touches_in_box": 195},

    {"player": "Phil Foden", "team": "Manchester City", "league": "Premier League", "position": "FW",
     "minutes": 2600, "goals": 12, "assists": 9, "xg": 10.5, "xa": 8.0,
     "shots": 65, "key_passes": 50, "dribbles_completed": 55,
     "progressive_carries": 120, "touches_in_box": 130},
    ...
    {"player": "Rafael Leao", "team": "AC Milan", "league": "Serie A", "position": "FW",
     "minutes": 2400, "goals": 13, "assists": 8, "xg": 11.8, "xa": 7.2,
     "shots": 68, "key_passes": 42, "dribbles_completed": 85,
     "progressive_carries": 155, "touches_in_box": 125},

    {"player": "Marcus Thuram", "team": "Inter Milan", "league": "Serie A", "position": "FW",
     "minutes": 2500, "goals": 16, "assists": 7, "xg": 14.2, "xa": 6.0,
     "shots": 70, "key_passes": 35, "dribbles_completed": 30,
     "progressive_carries": 60, "touches_in_box": 160},
    ...
    {"player": "Jamal Musiala", "team": "Bayern Munich", "league": "Bundesliga", "position": "FW",
     "minutes": 2300, "goals": 14, "assists": 9, "xg": 12.0, "xa": 8.5,
     "shots": 62, "key_passes": 58, "dribbles_completed": 65,
     "progressive_carries": 130, "touches_in_box": 135},

    {"player": "Florian Wirtz", "team": "Bayer Leverkusen", "league": "Bundesliga", "position": "FW",
     "minutes": 2600, "goals": 16, "assists": 15, "xg": 13.5, "xa": 12.8,
     "shots": 70, "key_passes": 78, "dribbles_completed": 60,
     "progressive_carries": 135, "touches_in_box": 140},
    ...
    {"player": "Randal Kolo Muani", "team": "Paris Saint-Germain", "league": "Ligue 1", "position": "FW",
     "minutes": 2200, "goals": 11, "assists": 6, "xg": 10.0, "xa": 5.2,
     "shots": 58, "key_passes": 30, "dribbles_completed": 25,
     "progressive_carries": 55, "touches_in_box": 150},
]
...
def simulate_shot_locations(player_row: pd.Series) -> pd.DataFrame:
    """Generate simulated shot (x, y) coordinates on a pitch for a player,
    for demo purposes. Pitch is 120 (length) x 80 (width), attacking left-to-right,
    with the goal at x=120. Real shot maps would use actual event data
    (e.g. from StatsBomb open data) instead of this simulation.
    """
    import numpy as np
    seed = abs(hash(player_row["player"])) % (2**32)
    rng = np.random.default_rng(seed)

    n_shots = int(player_row["shots"])
    n_goals = int(player_row["goals"])

    # Bias shot locations toward the box, with some spread further out
    x = rng.normal(loc=104, scale=10, size=n_shots).clip(75, 120)
    y = rng.normal(loc=40, scale=14, size=n_shots).clip(0, 80)

    shots_df = pd.DataFrame({"x": x, "y": y})
    # Mark a subset as goals (closest shots to goal, roughly)
    shots_df = shots_df.sort_values("x", ascending=False).reset_index(drop=True)
    shots_df["result"] = "No Goal"
    shots_df.loc[: max(n_goals - 1, -1), "result"] = "Goal"
    return shots_df
...
REAL_DATA_CSV = "real_player_stats.csv"

REAL_COLUMN_MAP = {
    "standard_pos": "position",
    "standard_age": "age",
    "standard_Playing Time_Min": "minutes",
    "standard_Performance_Gls": "goals",
    "standard_Performance_Ast": "assists",
    "shooting_Standard_Sh": "shots",
    "shooting_Standard_SoT": "shots_on_target",
    "misc_Performance_Crs": "crosses",
...
REAL_COUNTING_STATS = [
    "goals", "assists", "shots", "shots_on_target", "crosses",
    "fouls_drawn", "tackles_won", "interceptions",
]


def load_real_dataframe(csv_path: str = REAL_DATA_CSV) -> pd.DataFrame:
    """Load and clean the real FBref data pulled by fetch_real_data.py.
    Filters down to forwards with meaningful minutes played."""
    df = pd.read_csv(csv_path)

    keep_cols = ["league", "team", "player"] + list(REAL_COLUMN_MAP.keys())
    df = df[keep_cols].rename(columns=REAL_COLUMN_MAP)

    # Clean numeric columns (some come through as nullable/NA-containing)
    numeric_cols = ["age", "minutes"] + REAL_COUNTING_STATS
    for col in numeric_cols:
        df[col] = pd.to_numeric(df[col], errors="coerce").fillna(0)

    # Keep forwards only, with enough minutes to be meaningful
    df = df[df["position"].astype(str).str.contains("FW", na=False)]
    df = df[df["minutes"] >= 900]

    # Drop any rows missing key identifying info (can happen with header/junk rows)
    df = df.dropna(subset=["league", "team", "player"])
    ...
    df["league"] = df["league"].map(league_names).fillna(df["league"])

    return df.reset_index(drop=True)


def add_per90_stats_real(df: pd.DataFrame) -> pd.DataFrame:
    df = df.copy()
    per90_factor = 90 / df["minutes"].replace(0, pd.NA)
    for stat in REAL_COUNTING_STATS:
        df[f"{stat}_p90"] = (df[stat] * per90_factor).fillna(0).round(2)
    return df


def get_processed_data(use_real: bool = False, csv_path: str = REAL_DATA_CSV) -> pd.DataFrame:
    """Full pipeline: raw data -> per-90 -> percentile ranks.
    Set use_real=True to load from the CSV produced by fetch_real_data.py
    (falls back to sample data automatically if the file isn't found)."""
    if use_real and os.path.exists(csv_path):
        df = load_real_dataframe(csv_path)
        df = add_per90_stats_real(df)
        percentile_stats = REAL_COUNTING_STATS
